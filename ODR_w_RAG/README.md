...
##### Vector Database Search :file_cabinet:

Open Deep Research now supports vector database search for accessing internal documentation and knowledge bases. This allows the researcher to search through pre-embedded documents using semantic similarity.

To enable vector database search:
1. Set up your environment variables in `.env` (see [VECTORDB_USAGE.md](VECTORDB_USAGE.md) for details)
2. Enable `enable_vectordb_search` in the configuration
3. Prepare your embedding file (`embedding_semantic.json`) with pre-embedded documents

See [VECTORDB_USAGE.md](VECTORDB_USAGE.md) for detailed setup instructions. 
