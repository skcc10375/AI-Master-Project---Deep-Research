"""Debug runner for Deep Researcher.

이 스크립트를 실행하면 전체 그래프의 흐름을 추적할 수 있습니다.
VS Code의 디버거를 사용해서 중단점(breakpoint)을 설정하면 
각 단계마다 변수와 상태를 확인할 수 있습니다.
"""
...
async def debug_run():
    ...
    config = {
        ...
        "stream": False,
        "allow_clarification": False,  # 간단하게 하기 위해 clarification 스킵
    }
                        ...
                        if messages_list:
                            ...
                            if len(messages_list) > 1:
                                ...
                                for i, msg in enumerate(messages_list[:-1]):
                                    if hasattr(msg, 'type'):
                                        msg_preview = str(msg.content)[:50] if hasattr(msg, 'content') and msg.content else ""
